    ...
    <div className="grid grid-cols-1 xl:grid-cols-[1fr_360px] gap-8">
      <div>
        ...
        <div className="mt-8 rounded-2xl border border-white/8 bg-surface-900 overflow-hidden">
          <div className="px-6 py-4 border-b border-white/5">
            <p className="text-sm font-semibold text-white/85">Сводка</p>
          </div>
            ...
            {rows.map((r) => (
              <div key={r.title} className="px-6 py-5 flex items-start justify-between gap-6">
                <div>
                  ...
                  <p className="text-xs font-body text-white/35 mt-1">{r.meta}</p>
                </div>
                <p className="font-body tabular-nums text-sm font-semibold text-white">{r.amount}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <aside className="space-y-5">
        <div className="rounded-2xl border border-white/8 bg-surface-900 p-6">
          <p className="text-xs font-body text-white/35 uppercase tracking-[0.2em]">Комиссия</p>
