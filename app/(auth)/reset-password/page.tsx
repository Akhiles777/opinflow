...
function ResetPasswordPageContent() {
  ...
  if (!tokenState.checked) {
    return <div className="mx-auto mt-24 max-w-md rounded-2xl border border-white/8 bg-surface-900 p-10 text-center text-white">Проверяем ссылку...</div>;
  }
  ...
  if (!tokenState.valid) {
    return (
      <div className="mx-auto mt-16 max-w-md rounded-2xl border border-white/8 bg-surface-900 p-6 text-white sm:mt-24 sm:p-10">
        ...
        <p className="mt-3 text-sm leading-relaxed text-white/55">Запросите новую ссылку для восстановления пароля.</p>
        <Link href="/forgot-password" className="mt-8 inline-flex rounded-xl bg-brand px-6 py-3 text-sm font-semibold text-white transition-colors hover:bg-brand-dark">
  ...
  return (
    <div className="mx-auto mt-16 max-w-md rounded-2xl border border-white/8 bg-surface-900 p-6 text-white sm:mt-24 sm:p-10">
      ...
      <p className="mt-3 text-sm leading-relaxed text-white/55">После смены пароля все старые сессии будут завершены.</p>
      ...
      {state.success ? (
        ...
        <form action={formAction} className="mt-8 grid gap-4">
          ...
          <label className="grid gap-2">
            <span className="text-sm text-white/55">Новый пароль</span>
            <input name="password" type="password" className="h-11 rounded-xl border border-white/10 bg-white/5 px-3 text-sm text-white outline-none" />
          </label>
          <label className="grid gap-2">
