...
function ArrowBtn({ dir, onClick }: { dir: "prev" | "next"; onClick: () => void }) {
  return (
    <button type="button" onClick={onClick}
      className="flex h-10 w-10 items-center justify-center rounded-full border border-[#D4CBF0] dark:border-white/20 bg-white dark:bg-white/8 text-[#2C1A67] dark:text-white transition-all hover:border-[#B49DFF] hover:bg-[#F3EEFF]">
...
export default function RespondentTestimonials() {
  ...
  return (
    <section className="bg-[#FAFBFF] dark:bg-[#160840] px-4 py-16 lg:py-24">
      <div className="mx-auto max-w-[1400px] px-2 lg:px-6">
        <RevealOnScroll>
          <div className="flex items-center justify-between gap-6 mb-10 lg:mb-14">
            <h2 className="font-manrope text-[32px] sm:text-[44px] lg:text-[58px] leading-[0.95] tracking-[-0.04em] font-extrabold text-[#1C0C4C] dark:text-white">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 lg:gap-6">
          {visible.map((item, i) => (
            <RevealOnScroll key={`${item.name}-${i}`} delay={i * 80}>
              <div className="rounded-[24px] border border-[#E4DEF7] dark:border-white/8 bg-white dark:bg-white/6 p-6 lg:p-8 flex flex-col h-full min-h-[200px]">
                <div className="mb-2 text-[#6438D9] text-[28px] font-serif leading-none">&ldquo;</div>
                <p className="text-[15px] lg:text-[16px] leading-[1.65] text-[#35236B] dark:text-white/80 flex-1">{item.text}</p>
                <div className="mt-6 flex items-center gap-3">
                  ...
                  <div>
                    <div className="text-[15px] font-semibold text-[#1C0C4C] dark:text-white">{item.name}</div>
                    <div className="text-[13px] text-[#9B8FC9] dark:text-white/50">{item.meta}</div>
                  </div>
                </div>
              </div>
            </RevealOnScroll>
